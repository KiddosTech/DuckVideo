...
DuckVideo Studio is a desktop media utility for downloading and optimizing media locally. It uses Tauri to connect a Vue interface with native commands, allowing the app to run `yt-dlp`, FFmpeg, and the planned DuckVideo-CLI directly on the user's machine without a custom server.
...
- Tauri desktop app with local commands:
  ...
  - `download_media`
  - `compress_media`
- Preview system for planned media jobs
- Logs view for DuckVideo, `yt-dlp`, FFmpeg, and DuckVideo-CLI output
- DuckVideo-CLI command preview for the upcoming standalone/headless workflow
...
Vue UI
  -> Tauri command
    ...
    -> DuckVideo-CLI exposes the same workflow as headless commands
